...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        log_queries = lu.create_feature_log_queries(key, query_doc_ids, click_prior_query, self.featureset_name,
                                                    self.ltr_store_name,
                                                    terms_field=terms_field)
        # query_id appears to be passed as a pandas object.
        # Extract the value we will actually use.
        query_id_value = int(query_id.iloc[0])
        ...
        for query in log_queries:
            try:
                ...
                print('Error getting feature weights ({e:s}, q={q:s}).'.format(
                      e=str(re), q=str(query)))
                return None
            # Loop over the hits structure in response and extract out the
            # features per query_id and doc id.
            # Also capture and return all query/doc pairs that didn't
            # return features
            if (not response):
                continue
            ...
            for (ind, hit) in enumerate(hits):
                if (not hit):
                    continue
                fields = hit.get('fields', None)
                if (not fields):
                    continue
                featureLog = fields.get('_ltrlog', None)
                if (not featureLog):
                    continue
                featureLog = featureLog[0]
                if (not featureLog):
                    continue
                featureLog = featureLog.get('log_entry', None)
                if (not featureLog):
                    continue

                sku = hit.get('_source', None)
                if (sku):
                    sku = sku.get('sku', None)
                if (sku):
                    sku = sku[0]
                if (sku is None):
                    sku = -1
                ...
                docIds  .append(int(hit.get('_id', -1)))
                queryIds.append(query_id_value)
                skus    .append(sku)

                addFeatures = (not feature_results)
                for (ind, feature) in enumerate(featureLog):
                    featureName = feature.get('name' , None)
                    if (featureName is None):
                        continue
                    if (addFeatures):
                        featureValues                = list()
                        feature_results[featureName] = featureValues
                    else:
                        # Let this raise KeyError if there is no value.
                        featureValues = feature_results[featureName]
                    featureValues.append(feature.get('value', 0))

        if (0 >= len(docIds)):
            no_results[key] = query_doc_ids

        feature_results['doc_id'  ] = docIds
        feature_results['query_id'] = queryIds
        feature_results['sku'     ] = skus
